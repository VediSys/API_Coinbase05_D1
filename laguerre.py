...
""" laguerre.py | 70
    :param dataframe: df
    :param gamma: Between 0 and 1,
        default 0.75
    :return: Laguerre RSI: values 0 to +1
    - to trade: buy on the flat 0, sell on the drop from top, not when touch the top
    ...
    Vectorised pandas or numpy calculations
    are not used in Laguerre as L0 is
    self referencing. Uses intertuples loop.
    """
...
def LaGuerre(df,gamma): #orig df
    g = gamma; lrsi_l = []
    L0, L1, L2, L3 = 0.0, 0.0, 0.0, 0.0
    for row in df.itertuples(
        index=True, name='lrsi'):
